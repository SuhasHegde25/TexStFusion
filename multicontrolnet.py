...
class MultiControlNetModel(ModelMixin):
    ...
    def forward(
        ...
        if network_weightage is None:
            network_weightage = [1.0] * len(self.nets)    # Each controlnet gets weight of 1
        for i, (image, encoder_hidden_state, scale, controlnet) in enumerate(zip(controlnet_cond, encoder_hidden_states, conditioning_scale, self.nets)):
            ...
            # merge samples
            if i == 0:
                down_block_res_samples, mid_block_res_sample = [down_sample * network_weightage[i] for down_sample in down_samples], mid_sample * network_weightage[i]
            else:
                down_block_res_samples = [
                    samples_prev + samples_curr * network_weightage[i]
                    for samples_prev, samples_curr in zip(down_block_res_samples, down_samples)
                ]
                mid_block_res_sample += mid_sample * network_weightage[i]
